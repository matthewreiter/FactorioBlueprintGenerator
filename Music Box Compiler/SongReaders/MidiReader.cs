...
public static class MidiReader
{
    ...
    private const double HarmonicVelocityMultiplier = 0.8; // Reduce the volume of the harmonics so that the combined volume is closer to the original note
    ...
    public static Song ReadSong(string midiFile, bool debug, Dictionary<Instrument, int> instrumentOffsets, double masterVolume, Dictionary<Instrument, double> instrumentVolumes, bool allowInstrumentFallback, bool expandNotes, int? maxConcurrentNotes, FadeConfig fade)
    {
        ...
        {
            if (midiNote.StartTime - currentTime >= TickDuration || maxConcurrentNotes is not null && currentNotes.Count >= maxConcurrentNotes)
            {
